...
import { Game, PageProps, TopupProduct, User } from "@/types";
...
const Edit = ({ toptup }: EditProps) => {
    const [showModal, setShowModal] = useState(false);

    const { data, setData, errors, processing, reset, clearErrors } = useForm({
        product_name: toptup.product_name,
        price: toptup.price,
        details: toptup.details,
        game_id: toptup.game_id,
    });
    ...
    const submit = (e: SyntheticEvent) => {
        e.preventDefault();
        router.post(
            route("admin.topup-game.update", toptup.id),
            {
                ...data,
                _method: "put",
            },
